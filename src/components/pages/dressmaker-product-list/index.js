...
import { getDressmakerProducts } from 'services/api';
...
export default function ProductList(props){
    ...
    useEffect(() => {
        getDressmakerProducts(props.match.params.id, (res) => {
            setState(state => ({
                ...state, 
                filteredProducts: res.results.data,
                isLoading: false, 
            }))
        })
    },[])
    ...
    return (
        <div>
            {/*SEO Support*/}
            <Helmet>
                <title>Abooth | Couturier</title>
                <meta name="description" content="Abooph" />
            </Helmet>
            {/*SEO Support End */}

            <Breadcrumb title={state.currentParentCategory.nom || 'couturier'}/>
